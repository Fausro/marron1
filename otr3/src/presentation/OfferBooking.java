...
public class OfferBooking extends JFrame {
	...
	public OfferBooking() {
		...
		offerList.setBounds(99, 199, 243, 34);
		contentPane.add(offerList);
		offerList.setModel(offerInfo);
		offerList.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			//	System.out.println("P");
			//	if (e.getStateChange() == ItemEvent.SELECTED) {System.out.println("1");} // El evento se dispara dos veces: antes de cambiar el valor y una vez cambiado
				// Interesa s�lo actuar una vez cambiado
			//	if (offerList.isEnabled()){ 		// A este m�todo se le llama tambi�n cuando se hace un clear del JList, 
						   					 // por lo que podr�a estar la selecci�n vac�a y dar un error
				
					selectedOffer = (Offer) offerList.getSelectedItem();
					if(selectedOffer!=null && (selectedOffer.getDoubleNumber()>0 && doubleRooms.isSelected() || 
							selectedOffer.getTripleNumber()>0 && tripleRooms.isSelected() ||
							selectedOffer.getSingleNumber()>0 && singleRooms.isSelected())) {
					bookOffer.setEnabled(true);
					bookOffer.setText("Book this offer: "+selectedOffer);}else {
		...
		bookOffer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (error) bookOffer.setText("No available offer for that room type");
